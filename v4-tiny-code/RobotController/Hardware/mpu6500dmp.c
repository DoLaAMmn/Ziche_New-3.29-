...
uint8_t Get_MPU6500_DMP_Data(void){
    ...
    if (hal.new_gyro && hal.dmp_on) {
        ...
        if(sensors & INV_WXYZ_QUAT)
        {
            ...
            mpu_roll  = atan2(2*q2*q3+2*q0*q1,(-2)*q1*q1-2*q2*q2+1)*57.3;    //横滚角
            mpu_pitch = asin ((-2)*q1*q3+2*q0*q2)*57.3;                     //俯仰角
            mpu_yaw   = atan2(2*(q1*q2+q0*q3),q0*q0+q1*q1-q2*q2-q3*q3)*57.3;//偏航角
            
            printf("%.3lf,%.3lf,%.3lf\n",mpu_roll,mpu_pitch,mpu_yaw);//角度制
            //printf("%.0lf %.0lf %.0lf\r\n",roll,pitch,yaw);
